...
void ChinaMapWidget::openProvinceInfoDialog(const QString& provinceName) {
    provincerec* secondwin=new provincerec(NULL,provinceName,this->ID);
    secondwin->show();
}
...
void ChinaMapWidget::paintEvent(QPaintEvent* event)
{
    ...
    for (int i = 0; i < m_mapobjects.count(); i++) {
        ...
        int w = fm.horizontalAdvance(obj->name());
        QRect objNameRc;
        if (obj->name() == "澳门") {
    ...
    for (int i = 0; i < m_mapobjects.count(); i++) {
        ...
        for (int j = 0; j < areas.count(); j++) {
            ...
            if (pol.containsPoint(ptf, Qt::WindingFill)) {
                // 点击到了地图对象
                openProvinceInfoDialog(obj->name());
                return;
            }
        }
    }
